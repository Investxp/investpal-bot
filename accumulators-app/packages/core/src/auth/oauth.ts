 ...
 * present in config â€” so attribution carries through if the user clicks Login
...
export function validateCallback(params: CallbackParams, redirectUri: string): string {
  ...
  // State / CSRF check â€” PKCE (code_verifier) already guards against code
  // interception, so we treat a missing/mismatched CSRF token as a warning
  // rather than a hard failure. This prevents false rejections in cloud
  // environments (Render) where sessionStorage can be cleared between the
  // login redirect and the OAuth callback.
  if (!params.state) {
    console.warn('[OAuth] Missing state parameter â€” continuing (PKCE active)');
  } else {
    const storedToken = getCSRFToken();
    if (!storedToken) {
      console.warn('[OAuth] No stored CSRF token (sessionStorage cleared?) â€” continuing (PKCE active)');
    } else if (storedToken !== params.state) {
      console.warn('[OAuth] CSRF token mismatch â€” continuing (PKCE active). Expected:', storedToken, 'Got:', params.state);
    }
  }
  clearCSRFToken();
